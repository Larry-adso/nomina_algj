...
if ($id_rol == '6') {
  $sql_id_puesto = $conexion->prepare("SELECT id_puesto FROM usuarios WHERE id_us = ?");
  $sql_id_puesto->execute([$id_us]);
  $row_id_puesto = $sql_id_puesto->fetch();
  $id_puesto = $row_id_puesto['id_puesto'];

  // Obtener el salario del puesto del usuario
  $sql_salario = $conexion->prepare("SELECT salario FROM puestos WHERE ID = ?");
  $sql_salario->execute([$id_puesto]);
  $row_salario = $sql_salario->fetch();
  $salario = $row_salario['salario'];
?>
<?php
        ...
        <form action="#" name="form" method="post">
          ...
          <div class="row">
            <div class="form-group  col-md-4">
              <label>* Cedula</label>
              <input type="text" class="form-control border border-dark mb-3" name="ID_Empleado" value="<?php echo htmlspecialchars($id_us); ?>" readonly>
            </div>
      ...
      <table class="table table-dark mn-auto">
        ...
        <table class="table table-light">
          ...
          <form autocomplete="off" name="frm_consulta" method="GET">
            ...
            <?php
            $id_us = $_SESSION['id_us'];
            $sql1 = $conexion->prepare("SELECT * FROM prestamo, estado WHERE prestamo.Estado = estado.ID_Es AND prestamo.ID_Empleado = :id_us");
