...
<script>
    ...
    try {
      ...
      if (!isNaN(valor) && cantidadCuotas > 0) {
        ...
        if (valor > 0) {
          ...
          // Limpiar el campo de "Valor de las cuotas" si el valor es cero o está vacío
          document.getElementById('total').value = '';
        }
      } else {
        // Limpiar el campo de "Valor de las cuotas" si el valor o cantidad de cuotas no son válidos
        document.getElementById('total').value = '';
      }
    } catch (e) {
      console.error(e);
  ...
  function prepararEnvio() {
    ...
    valor1.value = valor1.value.replace(/[^\d]/g, '');
    total.value = total.value.replace(/[^\d]/g, '');
  }
</script>
      ...
      <table class="table table-dark mn-auto">
        <table class="table table-light">
          <form autocomplete="off" name="frm_consulta" method="GET">
            ...
            $sql1 = $conexion->prepare("SELECT prestamo.*, estado.estado AS estado_nombre FROM prestamo JOIN estado ON prestamo.estado = estado.ID_Es WHERE prestamo.ID_Empleado = :id_us");
